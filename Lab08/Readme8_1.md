...
## Конфигурации R1,2,3 представлены в виде txt в корне папки Lab08. 
...
# Часть 4:	Настройка пропускной способности и пассивных интерфейсов.
  ...
  ## Шаг 2:	Измените пропускную способность на маршрутизаторах.    
    ...
    d.  "R1#show ip route"  "D        10.2.2.0/30 [90/2681856] via 10.1.1.2, 01:06:06, Serial1/0", а было 2. (Часть 3, шаг 2) 
        ...
        Выполните на маршрутизаторе R1 команду show ip route. Появились ли изменения в таблице маршрутизации? Если да, в чем они заключаются? Из-за изменения скорости порта(не физическая, а для метрики EIGRP), изменилась метрика EIGRP для serial 1/1. И в результате был выбран только один маршрут в сеть 10.2.2.0/30. С наименьшей метрикой EIGRP.
